...
static void MAX44009_WriteReg(uint8_t addr, uint8_t value) {
	...
	buf[0] = addr;
	buf[1] = value;
    HAL_I2C_Master_Transmit(&hi2c2, MAX44009_ADDR, buf, 2, HAL_I2C_STATE_TIMEOUT); // HAL redo 0x02000000
...
static uint8_t MAX44009_ReadReg(uint8_t addr) {
	uint8_t value = 0;    
    HAL_I2C_Master_Transmit(&hi2c2, MAX44009_ADDR, &addr, 1, HAL_I2C_STATE_TIMEOUT); // HAL redo 0x00000000U
	// I2C_Transmit(MAX44009_I2C_PORT, &addr, 1, MAX44009_ADDR, I2C_GENSTOP_NO);

    HAL_I2C_Master_Receive(&hi2c2, MAX44009_ADDR, &value, 1, HAL_I2C_STATE_TIMEOUT);
